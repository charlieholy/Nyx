...
static inline uint64_t timeval_diff(struct timeval *old, struct timeval *ch_new)
{
    return (ch_new->tv_sec - old->tv_sec) * (1000 * 1000) - (old->tv_usec - ch_new->tv_usec);
}
...
static int inner_shift_log(dlog_t *log, struct timeval *now)
{
    ...
    for (i = num - 1; i >= 0; --i) {
        snprintf(path, PATH_MAX, "%s%s", log->base_name, log_suffix(log->shift_type, now->tv_sec, i));
        if (access(path, F_OK) == 0) {
            snprintf(ch_new_path, PATH_MAX, "%s%s", log->base_name, log_suffix(log->shift_type, now->tv_sec, i + 1));
            rename(path, ch_new_path);
        }
    }
