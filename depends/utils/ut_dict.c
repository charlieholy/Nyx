...
{
    dict_t ch_new;
    memcpy(&ch_new, dt, sizeof(dict_t));
    ch_new.size = dict_next_power(size);
    ch_new.mask = ch_new.size - 1;
    ch_new.table = calloc(ch_new.size, sizeof(dict_entry *));
    if (ch_new.table == NULL)
        return -1;
    ...
    for (uint32_t i = 0; i < dt->size && dt->used > 0; ++i) {
        ...
        while (entry) {
            ...
            uint32_t index = DICT_HASH_KEY(dt, entry->key) & ch_new.mask;
            entry->next = ch_new.table[index];
            ch_new.table[index] = entry;
            entry = next_entry;
            dt->used--;
        }
    }
    ...
    free(dt->table);
    memcpy(dt, &ch_new, sizeof(dict_t));
