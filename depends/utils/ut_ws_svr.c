...
static int on_http_message_complete(http_parser* parser)
{
    ...
    if (http_request_get_header(info->request, "Host") == NULL)
        goto error;
    double version;
    version = info->request->version_major + info->request->version_minor * 0.1;
    if (version < 1.1)
        goto error;
    const char *upgrade;
    upgrade = http_request_get_header(info->request, "Upgrade");
    if (upgrade == NULL || strcasecmp(upgrade, "websocket") != 0)
        goto error;
    const char *connection;
    connection = http_request_get_header(info->request, "Connection");
    if (connection == NULL)
        goto error;
    else {
    ...
    ws_version = http_request_get_header(info->request, "Sec-WebSocket-Version");
    if (ws_version == NULL || strcmp(ws_version, "13") != 0)
        goto error;
    const char *ws_key;
    ws_key = http_request_get_header(info->request, "Sec-WebSocket-Key");
    if (ws_key == NULL)
        goto error;
    const char *protocol_list;
    protocol_list = http_request_get_header(info->request, "Sec-WebSocket-Protocol");
    if (protocol_list && !is_good_protocol(protocol_list, svr->protocol))
        goto error;
    if (strlen(svr->origin) > 0) {
        const char *origin = NULL;
        origin = http_request_get_header(info->request, "Origin");
...
static int send_reply(nw_ses *ses, uint8_t opcode, void *payload, size_t payload_len)
{
    ...
    if (buf_size < require_len) {
        void *ch_new = realloc(buf, require_len);
        if (ch_new == NULL)
            return -1;
        buf = ch_new;
...
int ws_send_text_len(nw_ses *ses, char *message,size_t len)
{
    return send_reply(ses, 0x1, message, len);
}
